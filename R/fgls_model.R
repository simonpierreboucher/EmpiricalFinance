...
#' @param max_iter Maximum number of iterations for the FGLS estimation process.
...
#' @export
fgls_model <- function(data, max_iter = 100, tol = 1e-6) {
  ...
  # Itérer pour ajuster Sigma et les coefficients FGLS
  for (i in 1:max_iter) {
    # Estimer la nouvelle Sigma
    Sigma_new <- diag(as.vector(residuals^2))
    
    # Calculer les nouveaux coefficients FGLS
    Xt_Sigma_inv <- t(X) %*% solve(Sigma_new)
    beta_fgls <- solve(Xt_Sigma_inv %*% X) %*% Xt_Sigma_inv %*% y
    
    # Calculer les nouveaux résidus
    residuals_new <- y - X %*% beta_fgls
  ...
  # Calculer les erreurs standard des coefficients FGLS
  Xt_Sigma_inv_X_inv <- solve(Xt_Sigma_inv %*% X)
  se_beta_fgls <- sqrt(diag(Xt_Sigma_inv_X_inv))
  
  # Calculer les statistiques de test robustes
  t_values_robust <- beta_fgls / se_beta_fgls
