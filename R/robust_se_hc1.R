...
#' @export
robust_se_hc1 <- function(data, model_results) {
  ...
  beta <- model_results$coefficients$Value
  
  # Calculer les valeurs prédictes et les résidus
  y_hat <- X %*% beta
  residuals <- y - y_hat
  
  # Calculer la matrice de covariances robuste HC1
  n <- nrow(data)
  p <- ncol(X)
  X_t <- t(X)
  residuals_diag <- diag(as.vector(residuals^2))
  correction_factor <- n / (n - p)
  meat <- correction_factor * (X_t %*% residuals_diag %*% X)
